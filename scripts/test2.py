...
class DistanceCalculator:
    def __init__(self):
        ...
        self.scan_sub = rospy.Subscriber('/lds1/scan_raw', LaserScan, self.lidar_callback)
        ...
        self.marker_publisher2 = rospy.Publisher('/markers2', MarkerArray, queue_size=10)
...
############################### /// LIDAR_DETECT & CALCULATE /// ######################################
    ...
    def lidar_callback(self, msg):
        ...
        #         self.create_line_marker_length(x_avg1, y_avg1, x_avg2, y_avg2, marker_array)
        ...
        #         self.create_line_marker_width(x_avg1, y_avg1, x_outer1, y_outer1, marker_array2)
        ...
        #         self.marker_publisher2.publish(marker_array2)
        ...
        if len(grouped_points) >= 4:
            ...
            distance = self.calculate_distance((x_avg1, y_avg1), (x_avg2, y_avg2))
            distance2 = self.calculate_distance((x_avg3, y_avg3), (x_avg4, y_avg4))

            rospy.loginfo(f"Measured distance: {distance:.2f} meters")
            rospy.loginfo(f"Measured distance: {distance2:.2f} meters")

            if self.min_distance <= distance <= self.max_distance or self.min_distance <= distance2 <= self.max_distance:
                ...
                self.broadcast_group_transforms(grouped_points, quaternion)
                self.create_line_marker_length(x_avg1, y_avg1, x_avg2, y_avg2, marker_array)
                self.create_line_marker_length(x_avg1, y_avg3, x_avg2, y_avg3, marker_array)
                ...
                rospy.loginfo(f"Angle difference (degrees): {angle_diff_degrees:.2f}")
                outer_length = self.calculate_distance((x_avg2, y_avg2), (x_avg3, y_avg3))
                rospy.loginfo(f"outer_width: {outer_width:.2f}")
                rospy.loginfo(f"width: {outer_width:.2f}")
                rospy.loginfo(f"outer_length: {outer_length:.2f}")
                self.marker_publisher.publish(marker_array)
            self.last_object_time = rospy.Time.now()
        else:
            rospy.loginfo("Distance out of intensity")
############################### /// MOVEBASE_CONTROL /// ######################################
...
############################### /// PUB_MARKER /// #############################################
    ...
    def publish_intensity_markers(self, points, intensities):
        if not points or not intensities:
            ...
            marker.action = Marker.DELETEALL  # ถ้าไม่มีข้อมูลให้ลบ Marker
            self.marker_pub.publish(marker)
            return
    ...
    def create_marker(self, x, y, frame_id, marker_array):
        ...
        marker.scale.z = 0.05
        marker.color.a = 1.0
        marker.color.r = 1.0  # Example, can adjust based on frame_id
        marker.color.g = 0.0
        marker.color.b = 0.0
    ...
    def create_line_marker_length(self, x1, y1, x2, y2, marker_array):
        marker = Marker()
        marker.header.stamp = rospy.Time.now()
        marker.header.frame_id = 'lidar_merged_link'
    ...
    def create_line_marker_width(self, x1, y1, x2, y2, marker_array2):
        marker = Marker()
        marker.header.stamp = rospy.Time.now()
        marker.header.frame_id = 'lidar_merged_link'
        marker.ns = "line_connection"
