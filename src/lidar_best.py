...
from next_msgs.msg import (
    JackingOperationAction,
    JackingOperationGoal
)
class DistanceCalculator:
    def __init__(self):
        ...
        self.jack_client = actionlib.SimpleActionClient('next_jacking_ac_server', JackingOperationAction)
        self.jack_client.wait_for_server()
    ...
    def handle_shelf_4legs(self, clusters, msg):
        ...
        if (abs(dist_front - self.shelf_width) <= self.shelf_width_tolerance and
            ...
            yaw = math.atan2(front_center_y - back_center_y, front_center_x - back_center_x) + math.pi
    ...
    def navigate_to_shelf(self, x, y, yaw):
        ...
        goal.target_pose.pose.position.x = x - 0.2
        goal.target_pose.pose.position.y = y + 0.1
        quaternion = tf.transformations.quaternion_from_euler(0, 0, yaw)
        ...
        self.is_navigating = False
    def call_jack_load(self):
        rospy.loginfo("Now calling JackingOperationAction for JACK LOAD...")
        jack_goal = JackingOperationGoal()
        jack_goal.operation = JackingOperationGoal.JACK_LOAD
        jack_goal.speed = 2500
        jack_goal.timeout = 100
        self.jack_client.send_goal(jack_goal,
                                   feedback_cb=self.jack_feedback_cb)

        rospy.loginfo("Waiting for JACK LOAD result...")
        self.jack_client.wait_for_result()
        result = self.jack_client.get_result()
        rospy.loginfo(f"JACK LOAD Result: {result}")

    def jack_feedback_cb(self, feedback):
        rospy.loginfo(f"[JACK LOAD Feedback] seq={feedback.sequence}, text={feedback.text}")
